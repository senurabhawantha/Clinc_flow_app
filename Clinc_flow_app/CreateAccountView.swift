    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Button(action: { /* Dismiss or Navigate Back */ }) {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 32) {
                        ...
                        // MARK: - Form Fields
                        VStack(spacing: 24) {
                            inputField(label: "Full Name", placeholder: "Jane Doe", icon: "person", text: $fullName)
                            
                            inputField(label: "Email", placeholder: "jane@example.com", icon: "envelope", text: $email)
                                .keyboardType(.emailAddress)

                            passwordField(label: "Password", text: $password)
                            
                            // Confirm Password uses a different icon (arrow.counterclockwise.circle)
                            passwordField(label: "Confirm Password", text: $confirmPassword, icon: "arrow.counterclockwise")
                        }
                        ...
                        HStack(alignment: .top, spacing: 12) {
                            Button(action: { agreeToTerms.toggle() }) {
                                RoundedRectangle(cornerRadius: 4)
                                    ...
                                    .background(agreeToTerms ? Color.cyan : Color.white)
                                    .overlay(
                                        agreeToTerms ? Image(systemName: "checkmark").font(.system(size: 12, weight: .bold)).foregroundColor(.white) : nil
                                    )
                            }
    ...
    private func passwordField(label: String, text: Binding<String>, icon: String = "lock") -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                if label == "Password" { // Only show eye toggle for first password
